...
from emergency.apps.core.models import TrackPoint, Incident
from ..serializers import TrackPointSerializer, TrackPointCreateSerializer


class TrackPointCreateView(generics.CreateAPIView):
    """Crear un punto de tracking"""
    serializer_class = TrackPointCreateSerializer
    permission_classes = [IsAuthenticated]


class TrackPointBatchCreateView(APIView):
    ...
    def post(self, request):
        ...
        serializer = TrackPointSerializer(created_points, many=True)
...
class LastPositionView(APIView):
    ...
    def get(self, request):
        ...
        if incident_id:
            queryset = TrackPoint.objects.filter(incident_id=incident_id)
        else:
        ...
        points = TrackPoint.objects.filter(id__in=latest_ids).select_related('user')
        serializer = TrackPointSerializer(points, many=True)
...
class RouteView(APIView):
    ...
    def get(self, request):
        ...
        queryset = TrackPoint.objects.filter(
            user_id=user_id,
        ...
        serializer = TrackPointSerializer(queryset, many=True)
        return Response(serializer.data)
